...
public class ExcelFilterService {
  ...
  public List<StudentActivity> filterExcelForUploadedFilesEntries() {
    ...
    try {
      ...
      return studentActivities.stream()
          .filter(
              eventName -> FilterHelper.A_FILE_HAS_BEEN_UPLOADED.equals(eventName.getEventName()))
  ...
  public Map<String, Float> calculateAbsoluteFrequencyOfUploadedFiles() {
    ...
    uploadedFileActivities.forEach(
        entity ->
            ids.forEach(
                key -> {
  ...
  public Map<String, String> calculateRelativeFrequencyOfUploadedFiles() {
    ...
    filesUploadedPerUser
        .entrySet()
        .forEach(id -> id.setValue(id.getValue() / numberOfFilesUploaded * 100));
  ...
  public Float calculateScopeOfUploadedFiles() {
    List<Float> numberOfUploadedFiles =
        new ArrayList<>(calculateAbsoluteFrequencyOfUploadedFiles().values());

    return calculateAbsoluteFrequencyOfUploadedFiles().values().stream()
  ...
  public Set<String> findUniqueDescriptions() {
    ...
    uploadedFileActivities.forEach(
        studentActivity -> descriptions.add(studentActivity.getDescription()));
    return descriptions;
  }
